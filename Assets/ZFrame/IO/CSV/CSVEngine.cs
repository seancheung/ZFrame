...
namespace ZFrame.IO.CSV
{
    public class CSVEngine
    {
        ...
        public IEnumerable<T> Decode<T>() where T : new()
        {
            ...
            {
                if (i == _keyRow || _skippedRows != null && _skippedRows.Contains(i))
                    continue;
        ...
        {
            if (attribute.Index >= 0 && fields.Count > attribute.Index)
            {
                return fields[attribute.Index];
            }
            ...
            Debug.LogError(string.Format("Mapping Error! Index: {0}, Key: {1}, Name:{2}", attribute.Index,
        ...
        public bool Load<T>()
        {
            ...
            CSVDocumentAttribute document =
                Attribute.GetCustomAttribute(typeof (T), typeof (CSVDocumentAttribute), false) as CSVDocumentAttribute;
            _keyRow = document.MappingRow;
            _skippedRows = document.SkippedRows;

            if (string.IsNullOrEmpty(document.Path))
            {
                Debug.LogError(string.Format("CSV path not found: {0}", document.Path));
                return false;
            }

            //Read text
            TextAsset asset = Resources.Load<TextAsset>(document.Path);

            if (asset == null)
            {
                Debug.LogError(string.Format("CSV file not found: {0}", document.Path));
                return false;
            }

            string content = asset.text;
            bool result = Load(content);
            ...
            if (result)
            {
                if (_keyRow < 0 || _records[_keyRow].Any(string.IsNullOrEmpty))
                {
                    Debug.LogError(
                        string.Format(
                            "Encoding Error! No key column found. Make sure target file is in UTF-8 format. Path: {0}",
                            document.Path));
                    return false;
                }
            }
        ...
        /// <summary>
        /// Get string value at specified row and column. If record empty or position not found, NULL will be returned. Row/Index starts at 0
        /// </summary>
        ...
        /// <summary>
        /// Get a converted value at specified row and column. If record empty or position not found or convertion failed, defaultValue will be returned. Row/Index starts at 0
        /// </summary>
